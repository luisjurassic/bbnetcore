...
/// <summary>
/// Classe para conversão do enumerador <see cref="SituacaoBoleto"/> para json e vice-versa.
/// </summary>
public class SituacaoBoletoConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(SituacaoBoleto), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out SituacaoBoleto situacao);

        writer.WriteValue(((int)(parse ? situacao : SituacaoBoleto.Normal)).ToString());
    }
}