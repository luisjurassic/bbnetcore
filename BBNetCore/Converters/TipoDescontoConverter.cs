...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoDesconto"/> para json e vice-versa.
/// </summary>
public class TipoDescontoConverter : JsonConverter
{
    /// <summary>
    ...
    {
        var parse = Enum.ToObject(typeof(TipoDesconto), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoDesconto tipoDesconto);

        writer.WriteValue(((int)(parse ? tipoDesconto : TipoDesconto.SemDesconto)).ToString());
    }
}