...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoMulta"/> para json e vice-versa.
/// </summary>
public class TipoMultaConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoMulta), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoMulta tipo);

        writer.WriteValue(((int)(parse ? tipo : TipoMulta.SemMulta)).ToString());
    }
}