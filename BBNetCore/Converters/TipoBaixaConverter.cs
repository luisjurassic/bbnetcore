...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoBaixa"/> para json e vice-versa.
/// </summary>
public class TipoBaixaConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoBaixa), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoBaixa situacao);

        writer.WriteValue(((int)(parse ? situacao : TipoBaixa.BaixadoPorSolicitacao)).ToString());
    }
}