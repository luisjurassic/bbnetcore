...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoJurosMora"/> para json e vice-versa.
/// </summary>
public class TipoJurosMoraConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoJurosMora), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoJurosMora tipo);

        writer.WriteValue(((int)(parse ? tipo : TipoJurosMora.Isento)).ToString());
    }
}