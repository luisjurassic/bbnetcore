...
/// <summary>
/// Classe para conversão do enumerador <see cref="FormaPagamento"/> para json e vice-versa.
/// </summary>
public class FormaPagamentoConverter : JsonConverter
{
    /// <summary>
    ...
    public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
    {
        ...
        if (string.IsNullOrWhiteSpace(value))
            return FormaPagamento.PagamentoPendente;
        
        var parse = Enum.ToObject(typeof(FormaPagamento), value[0]);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out FormaPagamento forma);

        writer.WriteValue(((int)(parse ? forma : FormaPagamento.PagamentoPendente)).ToString());
    }
}