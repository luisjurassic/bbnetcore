...
/// <summary>
/// Classe para conversão do enumerador <see cref="NaturezaRecebimento"/> para json e vice-versa.
/// </summary>
public class NaturezaRecebimentoConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(NaturezaRecebimento), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out NaturezaRecebimento naturezaRecebimento);

        writer.WriteValue(((int)(parse ? naturezaRecebimento : NaturezaRecebimento.Normal)).ToString());
    }
}