...
/// <summary>
/// Classe para conversão do enumerador <see cref="OcorrenciasCartorio"/> para json e vice-versa.
/// </summary>
public class OcorrenciasCartorioConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(OcorrenciasCartorio), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out OcorrenciasCartorio ocorrencias);

        writer.WriteValue(((int)(parse ? ocorrencias : OcorrenciasCartorio.TituloProtocoladoTec)).ToString());
    }
}