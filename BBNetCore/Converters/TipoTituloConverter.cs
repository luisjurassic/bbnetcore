...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoTitulo"/> para json e vice-versa.
/// </summary>
public class TipoTituloConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoTitulo), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoTitulo tipoTitulo);

        writer.WriteValue(((int)(parse ? tipoTitulo : TipoTitulo.Outros)).ToString());
    }
}