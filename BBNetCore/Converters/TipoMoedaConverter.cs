...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoMoeda"/> para json e vice-versa.
/// </summary>
public class TipoMoedaConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoMoeda), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoMoeda moeda);

        writer.WriteValue(((int)(parse ? moeda : TipoMoeda.Nenhum)).ToString());
    }
}