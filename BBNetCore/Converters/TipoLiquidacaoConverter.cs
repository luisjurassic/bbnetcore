...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoLiquidacao"/> para json e vice-versa.
/// </summary>
public class TipoLiquidacaoConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoLiquidacao), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoLiquidacao tipo);

        writer.WriteValue(((int)(parse ? tipo : TipoLiquidacao.Caixa)).ToString());
    }
}