...
/// <summary>
/// Classe para conversão do enumerador <see cref="TipoInscricao"/> para json e vice-versa.
/// </summary>
public class TipoInscricaoConverter : JsonConverter
{
    /// <summary>
    ...
    { 
        var parse = Enum.ToObject(typeof(TipoInscricao), reader.Value);
        return parse;
    }
    ...
    {
        var parse = Enum.TryParse(value?.ToString() ?? string.Empty, out TipoInscricao tipo);

        writer.WriteValue(((int)(parse ? tipo : TipoInscricao.CPF)).ToString());
    }
}